...
measurement error. The data set we will work on, is the
movement track of three narwhals tagged with a Fastlock GPS tag for two weeks, provided by Dr. Marianne Marcoux.For simplicity, we only examine the GPS data from two weeks in August 2017 for three individuals. We only use the fastloc GPS data, so we don't have to deal with location error.
</p>
...
# Handling data gaps 
There are several ways to deal with data gaps, but we will focus on the following option using path segmentation. The basic steps are:

1. Splitting tracks
2. Interpolating locations within each track segment

## Splitting tracks
The basic idea of path segmentation is to split the track at points where there are very large data gaps (i.e., assign each track segment a new individual ID). This may be particularly appropriate for gaps where we may reasonably expect that the underlying states are effectively independent of one another, or long enough gaps that we don't expect any interpolation method to perform well. We can split the tracks for gaps larger than a predetermined threshold by iterating the ID column. Here, we will use a function (found in `utility_functions.R`) to split the tracks. We define the maximum allowable gap (at which point it will start a new segment), as well as the shortest allowable track segment. An alternative to this approach is to fill large gaps with NAs, but this can be problematic if there is any covariate-dependence on the transition probabilities.
...
Notice how the predicted tracks do not make perfectly straight lines through missing sections. Next, we need to derive step lengths and turning angles using the `prepData` function. We don't have any covariates, but if we did, we would specify them here so that the function treats them as such (rather than as ab observation variable). The default is UTM, so we actually don't need to specify it, but we do so for clarity in this example.
```{r prep}
...
<p style='text-align: justify;'> 
We can also use the `plot` function to plot the outputs. This will show us the estimated state-dependent distributions, and estimated states for each location. The locations are plotted for each ID separately (of which we have many), and so we will only plot one for simplicity.
</p>
...
You can plot the entire time series (for each ID) from both of these functions using the function `plotStates`. Here, we plot the locations, colored by their viterbi states.
...
This indicates the the model with a time-varying mean step length is the (marginally) best fitting model. 
